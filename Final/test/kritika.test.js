...
contract(SupplyChain,([deployer, manufacturer, retailer, transporter, customer])=>{
    ...
    describe('Adding and sellig item', async()=>{
        ...
        before(async ()=>{
            result = await supplyChain.giveOrderToManufacturer('thread', 1,manufacturer,{from: deployer, value: web3.utils.toWei('1', 'Ether')})
            const event = result.logs[0].args;
            assert.equal(event.name, "thread",'item name is correct');
        })
